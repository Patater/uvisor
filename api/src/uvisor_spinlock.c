...
{
    uvisor_export_table()->spinlock.init(spinlock);
}

bool uvisor_spin_trylock(UvisorSpinlock * spinlock)
{
    return uvisor_export_table()->spinlock.trylock(spinlock);
}

void uvisor_spin_lock(UvisorSpinlock * spinlock)
{
    uvisor_export_table()->spinlock.lock(spinlock);
}

void uvisor_spin_unlock(UvisorSpinlock * spinlock)
{
    uvisor_export_table()->spinlock.unlock(spinlock);
}
