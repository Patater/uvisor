...
void __uvisor_lib_box_init(void * lib_config)
{
    ...
    /* The thread definition is most likely in flash, so we need to copy it to
     * box-local RAM before we can modify it. Allocate some RAM for storing the
     * main thread's thread definition. This memory is never freed. */
    thread_def = malloc_p(sizeof(*thread_def));
    if (thread_def == NULL) {
        /* No process heap memory available */
        mbed_die();
    }

    /* Copy thread definition from flash to RAM. */
