    ...
    size_t bytes)
{
    ...
    /* The internal rt_Memory memory pool structure must be placed AFTER
     ...
     * the size) */
    size &= -0x8;
    /* XXX Does secure_allocator_create_with_pool guarantee at least bytes
     * number of bytes available? I think it does not, as it was subtracting
     * from bytes before when making the RTX allocator. The effect is that RTX2
     * has more overhead. */

    /* Create pool allocator structure inside the memory. */
    if (!os_MemoryInit((void *) page_origin, size)) {
        /* Abort if failed. */
    ...
    size_t maximum_malloc_size)
{
    ...
    /* The rt_Memory allocator puts one pool allocator structure at both the
    ...
    for(size_t ii = 0; ii < page_count; ii++) {
        ...
        DPRINTF("secure_allocator_create_with_pages: Created memory pool allocator %p with offset %d\n", allocator->table.page_origins[ii], 0);
    }
    DPRINTF("\n");
    ...
    void * ptr)
{
    ...
    do {
        /* Search in this page. */
        int ret = os_MemoryFree(table(allocator)->page_origins[index], ptr);
