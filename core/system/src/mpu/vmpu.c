...
static int vmpu_sanity_checks(void)
{
    ...
    /* Note: All labels in the BSS section in the host linker script are aligned
     * to 32 bytes. */
    assert((uint32_t) __uvisor_config.bss_start == UVISOR_ROUND32_UP((SRAM_ORIGIN + SRAM_OFFSET)));
     ...
     * UVISOR_SRAM_LENGTH. */
    assert(VMPU_REGION_SIZE(__uvisor_config.bss_main_start, __uvisor_config.bss_main_end) >= UVISOR_SRAM_LENGTH);
    assert(&__stack_end__ > __uvisor_config.bss_main_start);
